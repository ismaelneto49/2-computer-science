...
class BuscaTest extends BaseTest {
    ...
    void testBuscaAvancadaDoisMetadados() {
        ...
        DocumentoDTO[] busca = this.buscaController.busca(metadadosBuscados);
        assertEquals(2, busca.length, "Documentos de texto de 1 linha");
        assertEquals(TEXTO3_ID, busca[0].getId());
    }

    @Test
    void testBuscaAvancadaDocumentoHtml() {
        Map<String, String> metadadosBuscados = new HashMap<>();
        metadadosBuscados.put("TIPO", "html");
        DocumentoDTO[] busca = this.buscaController.busca(metadadosBuscados);
        assertEquals(1, busca.length);
        assertEquals(HTML_ID, busca[0].getId());
    }
}
